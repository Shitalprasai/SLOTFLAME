...
import { AtmosphericLeftStage } from './AtmosphericLeftStage';
...
export const SlotGame: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* 3. MAIN GAMEPLAY VIEWPORT: LEFT STAGE (Jackpot Progressive Tower) + RIGHT 3D SLOT MACHINE (Expanded Reels!) */}
      <main
        id="slot-main-stage"
        className="relative z-20 w-full flex-1 min-h-0 flex flex-col landscape:flex-row sm:flex-row items-center justify-between gap-2 sm:gap-4 md:gap-6 px-1 sm:px-3 md:px-6 my-auto overflow-visible"
      >
        {/* Left Area: Progressive Jackpot Marquee, Rules, & Fire Lounge Atmosphere */}
        <AtmosphericLeftStage
          currentBet={currentBet}
          winTier={activeResult?.winTier ?? null}
        />

        {/* Right Area: 3D Slot Machine with Huge, Uncompressed Cylindrical Reels & Multiplier Box */}
        <div className="relative flex-1 h-full min-h-0 flex items-center justify-center landscape:justify-end sm:justify-end max-w-2xl lg:max-w-3xl xl:max-w-4xl">
          <SlotCabinet3D
            reels={reels}
      ...
      {/* 4. LANDSCAPE BOTTOM BAR: All buttons docked horizontally across the landscape bottom */}
      <div className="relative z-30 w-full max-w-6xl shrink-0 px-1 sm:px-2 pb-0.5 sm:pb-1">
        <GameControls
          balance={balance}
          bet={currentBet}
          win={win}
