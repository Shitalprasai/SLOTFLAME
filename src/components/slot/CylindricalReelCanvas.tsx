...
export const CylindricalReelCanvas: React.FC<CylindricalReelCanvasProps> = ({
  ...
  const renderFrame = useCallback(
    (timestamp: number, dt: number) => {
      ...
      // Reel Geometry calculations - strictly responsive for mobile and desktop
      const reelCount = 3;
      const isMobile = width < 480;
      const dividerWidth = isMobile ? Math.max(3, Math.round(width * 0.012)) : Math.max(6, Math.round(width * 0.02));
      const totalDividers = (reelCount - 1) * dividerWidth;
      const sidePadding = isMobile ? Math.max(2, Math.round(width * 0.008)) : Math.max(4, Math.round(width * 0.015));
      const availableReelWidth = width - sidePadding * 2 - totalDividers;
      const reelWidth = availableReelWidth / reelCount;

      const centerY = height / 2;
      // Cylinder radius & row pitch: ensures 3 rows are spaced cleanly and never squished on mobile
      ...
      // Prevents vertical collision on small heights while maximizing size on mobile
      const maxW = reelWidth * (isMobile ? 0.88 : 0.84);
      const maxH = rowPitch * 1.08;
      const baseSize = Math.max(24, Math.min(maxW, maxH));
      ...
      for (let r = 0; r < reelCount; r++) {
        ...
        // Scaled dynamically to never overlap or obscure Row 0 or Row 2 on mobile devices!
        const hoodDepth = Math.max(10, height * 0.12);
        const hoodDip = Math.max(16, height * 0.17);
